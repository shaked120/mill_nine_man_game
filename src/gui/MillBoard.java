...
public class MillBoard extends JPanel {
	...
	protected void paintComponent(Graphics g) {
		...
		for (int i = 0; i < 24; i++) {
			...
			if (!board.getHouses().get(i).isEmpty()) {
				...
				} else if (board.getHouses().get(i).getMan().getColor() == mill_classes.Color.White
						|| (jump != null && jump.getDestinationId() == i && board.getCurrentPlayer().getColor() == mill_classes.Color.White)) {
	...
	private class Controller extends MouseAdapter {
		...
		public void mouseClicked(MouseEvent e) {
			if (!doMakeJump || board.getCurrentPlayer().getColor() == mill_classes.Color.Black || board.hasCurrentPlayerLost()) {
				return;
			}
